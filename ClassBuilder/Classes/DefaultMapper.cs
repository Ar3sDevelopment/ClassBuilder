...
namespace ClassBuilder.Classes
{
    public class DefaultMapper<TSource, TDestination> : IMapper<TSource, TDestination>
    {
        ...
        public TDestination Map(TSource source, TDestination destination)
        {
            ...
            {
                foreach (var p in customProperties.Where(t => Attribute.GetCustomAttribute(t, typeof(MapEqualsAttribute)) != null))
                    IterEqualsProperties(source, destination, p);
            }
        ...
        private void IterGeneric(TDestination destination, string name, object value) => typeof(TDestination).GetProperty(name)?.SetValue(destination, value);
        ...
        private void IterMapProperties(TSource source, TDestination destination, PropertyInfo propertyInfo, MapFieldAttribute attribute)
        {
            if (attribute != null)
                IterGeneric(destination, attribute.ToField, propertyInfo.GetValue(source));
        }
